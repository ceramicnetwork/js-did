...
import * as mapper from '../src/nist_weierstrass_common.js'
import * as u8a from 'uint8arrays'

test('a hex string with unexpected input', () => {
  const inputPublicKeyHex = ''
  const publicKey_u8a = mapper.testHexString(inputPublicKeyHex)
  expect(publicKey_u8a).toEqual(false)
})

test('a hex string with unexpected input : try 2', () => {
  const inputPublicKeyHex = 99
  const publicKey_u8a = mapper.testHexString(inputPublicKeyHex)
  expect(publicKey_u8a).toEqual(false)
})

test('a hex string shorter than 33 bytes', () => {
  const inputPublicKeyHex = 'abc09'
  const publicKey_u8a = mapper.testHexString(inputPublicKeyHex)
  expect(publicKey_u8a).toEqual(true)
})

test('testUint8Array with correct input', () => {
  const inputCompressedPoint = Uint8Array.from([
    3, 127, 35, 88, 48, 221, 61, 239, 167, 34, 239, 26, 162, 73, 214, 160, 221, 187, 164, 249, 144,
    176, 129, 117, 56, 147, 63, 87, 54, 64, 101, 53, 66,
  ])
  const publicKey_u8a = mapper.testUint8Array(inputCompressedPoint)
  expect(publicKey_u8a).toEqual(true)
})

test('testUint8Array with number', () => {
...
test('expect pubKeyBytesToHex to convert byte array to hex string', () => {
  ...
  const input = Uint8Array.from([
    128, 36, 127, 35, 88, 48, 221, 61, 239, 167, 34, 239, 26, 162, 73, 214, 160, 221, 187, 164, 249,
    144, 176, 129, 117, 56, 147, 63, 87, 54, 64, 101, 53, 66, 133, 109, 168, 141, 51, 95, 31, 178,
    91, 139, 207, 190, 8, 149, 40, 220, 224, 155, 31, 124, 185, 159, 221, 96, 248, 131, 0, 244, 194,
    204, 109, 53,
  ])
...
test('a compressed public key in hex with an odd number of characters', () => {
  const inputPublicKeyHex = '2f9c36f8964623378bdc068d4bce07ed17c8fa486f9ac0c2613ca3c8c306d7bb6'
  const output = Uint8Array.from([
    2, 249, 195, 111, 137, 100, 98, 51, 120, 189, 192, 104, 212, 188, 224, 126, 209, 124, 143, 164,
    134, 249, 172, 12, 38, 19, 202, 60, 140, 48, 109, 123, 182,
  ])
  const publicKey_u8a = pubKeyHexToUint8Array(inputPublicKeyHex)
...
test('convert a public key x,y where x and y are integers to a pair of Uint8Arrays', () => {
  ...
  const output = {
    xOctet: Uint8Array.from([
      249, 195, 111, 137, 100, 98, 51, 120, 189, 192, 104, 212, 188, 224, 126, 209, 124, 143, 164,
      134, 249, 172, 12, 38, 19, 202, 60, 140, 48, 109, 123, 182,
    ]),
    yOctet: Uint8Array.from([
      28, 211, 103, 23, 184, 172, 94, 79, 234, 138, 210, 61, 200, 208, 120, 60, 35, 24, 238, 74,
      215, 168, 13, 182, 224, 2, 106, 208, 176, 114, 162, 79,
    ]),
  }
...
test('key compression (y-coordinate even)', () => {
  ...
  const output = Uint8Array.from([
    2, 57, 195, 221, 116, 19, 23, 41, 68, 109, 193, 179, 218, 103, 212, 159, 192, 70, 252, 191, 7,
    47, 204, 91, 159, 165, 28, 5, 185, 116, 48, 127, 150,
  ])
...
test('key compression (y-coordinate odd)', () => {
  const inputPublicKeyHex =
    'f9c36f8964623378bdc068d4bce07ed17c8fa486f9ac0c2613ca3c8c306d7bb61cd36717b8ac5e4fea8ad23dc8d0783c2318ee4ad7a80db6e0026ad0b072a24f'
  const output = Uint8Array.from([
    3, 249, 195, 111, 137, 100, 98, 51, 120, 189, 192, 104, 212, 188, 224, 126, 209, 124, 143, 164,
    134, 249, 172, 12, 38, 19, 202, 60, 140, 48, 109, 123, 182,
  ])
...
test('a null coordinates in ECPointCompress', () => {
  const x = null
  const y = null
  expect(() => {
    ECPointCompress(x, y)
...
test('a undefined coordinates in ECPointCompress', () => {
  const x = undefined
  const y = undefined
  expect(() => {
    ECPointCompress(x, y)
...
test('null in pubKeyHexToUint8Array', () => {
...
test('undefined in pubKeyHexToUint8Array', () => {
...
test('an empty string in pubKeyHexToUint8Array', () => {
  const inputPublicKeyHex = ''
  const publicKey_u8a = pubKeyHexToUint8Array(inputPublicKeyHex)
...
test('compresedKeyInHex with an empty string', () => {
  const inputPublicKeyHex = ''
...
test('compresedKeyInHex with null', () => {
  const inputPublicKeyHex = null
  expect(() => {
...
test('compresedKeyInHex with undefined', () => {
  const inputPublicKeyHex = undefined
  expect(() => {
    compresedKeyInHex(inputPublicKeyHex)
  }).toThrowError('input cannot be null or undefined.')
