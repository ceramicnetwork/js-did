...
test('a uncompressed public key in hex to an x,y point with x, and y url with an unsupported prefix', () => {
...
test('a uncompressed public key in hex to an x,y point with x, and y url encoded with an unsupported prefix that is too long', () => {
  const inputPublicKeyHex =
...
test('a compressed public key in hex to an x,y point with x, and y url encoded with an unsupported prefixi: try2', () => {
  const inputPublicKeyHex =
    '050e167184c0436f5940e63d89f3827bb90978cbd3b26ef66d52da7638ce0f83492fe3f27794d8dd75a8b9553161b8613a'
  const publicKey_u8a = pubKeyHexToUint8Array(inputPublicKeyHex)
  expect(() => {
    mapper.pubKeyBytesToXY(publicKey_u8a)
  }).toThrowError('Unexpected pubKeyBytes')
...
test('a compressed public key in hex to an x,y point with x, and y url encoded with an unsupported prefixi: try3', () => {
  const inputPublicKeyHex =
...
test('a compressed public key in hex to an x,y point with x, and y url encoded with an unsupported prefix', () => {
  const inputPublicKeyHex =
    '040e167184c0436f5940e63d89f3827bb90978cbd3b26ef66d52da7638ce0f83492fe3f27794d8dd75a8b9553161b8613a'
  const publicKey_u8a = pubKeyHexToUint8Array(inputPublicKeyHex)
  expect(() => {
...
test('a compressed public key in hex to an x,y point with x, and y url encoded with an unexpected length', () => {
  const inputPublicKeyHex =
...
test('a hex string longer than 97 bytes', () => {
  const inputPublicKeyHex =
...
test('a hex string longer than 97 bytes: try2', () => {
  const inputPublicKeyHex =
...
test('a compressed public key in hex to an x,y point with x, and y url encoded', () => {
  const inputPublicKeyHex =
...
test('a uncompressed public key in hex to an x,y point with x, and y url encoded', () => {
  const inputPublicKeyHex =
...
test('a raw public key in hex to an x,y point with x, and y url encoded', () => {
  const inputPublicKeyHex =
...
test('a compressed public key in hex to an x,y point as BigInt', () => {
  const inputPublicKeyHex =
...
test('key decompression (y-coordinate even)', () => {
  const inputCompressedPoint = Uint8Array.from([
    2, 35, 113, 197, 135, 41, 168, 153, 179, 78, 104, 37, 216, 27, 218, 187, 126, 99, 72, 18, 196,
    175, 235, 162, 201, 186, 142, 10, 149, 170, 210, 194, 49, 95, 248, 223, 21, 242, 90, 255, 4, 44,
    200, 42, 72, 183, 130, 241, 36,
  ])
  const output = {
...
test('key decompression (y-coordinate odd)', () => {
  const inputCompressedPoint = Uint8Array.from([
    3, 85, 60, 88, 67, 106, 249, 139, 178, 116, 167, 2, 233, 109, 11, 235, 188, 203, 103, 93, 18,
    204, 150, 41, 122, 192, 55, 55, 55, 99, 127, 81, 127, 209, 143, 40, 136, 37, 1, 46, 95, 243,
    141, 178, 93, 39, 156, 202, 42,
  ])
  const output = {
...
test('key decompression (y-coordinate odd) key#2', () => {
  const inputCompressedPoint = Uint8Array.from([
    3, 171, 37, 12, 72, 230, 28, 207, 37, 101, 156, 140, 69, 139, 233, 213, 25, 26, 99, 179, 80, 88,
    236, 18, 49, 84, 35, 158, 171, 117, 128, 162, 150, 77, 208, 229, 89, 196, 110, 214, 41, 237, 36,
    9, 182, 118, 204, 121, 43,
  ])
