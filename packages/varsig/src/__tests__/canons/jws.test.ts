...
const testJws =
  'eyJhbGciOiAiRVMyNTYifQ.eyJpYXQiOjE1MTYyMzkwMjIsIm5hbWUiOiJKb2huIERvZSIsInN1YiI6IjEyMzQ1Njc4OTAifQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c'

const expectedHashInput = uint8arrays.fromString(testJws.slice(0, testJws.lastIndexOf('.')))
...
test('Encode jws with unordered fields fails', () => {
  ...
  expect(() => fromOriginal(unorderedJws)).toThrow(
    /Invalid JOSE payload: Varsig only supports JSON with ordered keys, got/,
  )
})
