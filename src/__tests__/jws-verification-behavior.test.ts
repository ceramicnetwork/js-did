...
describe('atTime', () => {
  ...
  test('ok before rotation', async () => {
    const beforeRotation = new Date('2021-06-21T10:20:46Z').valueOf()
    const { kid } = await did.verifyJWS(jws, beforeRotation)
    expect(kid).toMatchSnapshot()
  })
  test('fail after rotation', async () => {
    const afterRotation = new Date('2021-06-22T10:20:46Z').valueOf()
    await expect(did.verifyJWS(jws, afterRotation)).rejects.toThrow(/Key is revoked/)
  })
})
